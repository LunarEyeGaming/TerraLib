...
function init()
    mcontroller.setVelocity({0.01, 0})
  self.pathing = {}
...
repeat
  ...
  if world.gravity(mcontroller.position()) == 0 then
    ...
    if not self.inGround then
        -- if not, check if we're in a liquid
        if world.liquidAt({math.floor(mcontroller.position()[1]+0.5), math.floor(mcontroller.position()[2]+0.5)}) then
            -- if so, and if we are meant to do so, treat this liquid as ground
            self.inGround = config.getParameter("treatLiquidAsGround", true) -- previously false, now is true since I found out, ALL worms can dig through liquids in Terraria. Doing this just saves me the time of changing it for every worm individually.
        end
    end
  end
  -- calculate gravity
...
function move()
    ...
    function postMove()
        if home then
            world.debugText("home", home, "green")
        else
            world.debugText("no home", mcontroller.position(), "green")
        end
    ...
    if ownerId() then
        home = world.entityPosition(ownerId())
    end
    if not self.targetId then
        if not home then
            home = mcontroller.position()
        end
        if world.magnitude(home, mcontroller.position()) <= 10 then
            if not self.inGround then
    ...
    if not ownerId() and self.targetId then
        home = nil
    end
    if self.targetId and world.magnitude(home or mcontroller.position(), mcontroller.position()) <= 50  then
