...
function setPosition(p, upd)
    armPosition = p
    if upd then
      mcontroller.setPosition(vec2.addToRef(armPosition, vec2.mulToRef(vec2.withAngle(mcontroller.rotation(),armLength), {mcontroller.facingDirection(), 1}, workVec21), workVec22))
    end
end
function isHeld()
  return true
