    ...
    if not continue and (not chain.targetEntityId or world.entityExists(chain.targetEntityId)) then
      ...
      if segmentCount > 0 then
        ...
        for i = 1, lastDrawnSegment do
          ...
          local cropperc = (chainLength / chain.segmentSize) - lastDrawnSegment + 0.5
          
          local thisOffsetPerc = 1
          local isLastFullSegment = (i == lastDrawnSegment and not chain.endSegmentImage) or (i == lastDrawnSegment - 1 and chain.endSegmentImage)
          if chain.crop then
              if isLastFullSegment then
                 image = image.."?crop=0;0;"..math.floor(chain.crop.imageWidth * cropperc)..";"..chain.crop.imageHeight
                 thisOffsetPerc = cropperc
              end
          end
          ...
          elseif chain.scale then
              image = image.."?scale="..chain.scale
          end
          ...
          local segmentPos = vec2.add(vec2.sub(currentBaseOffset, isLastFullSegment and chain.crop and vec2.mul(segmentOffset, (1 - cropperc) / 2) or {0, 0}), thisOffset)
          local drawable = {
            image = image,
            centered = true,
          ...
          currentBaseOffset = vec2.add(currentBaseOffset, vec2.mul(segmentOffset, thisOffsetPerc))
        end
      end
    end
  end
