...
self.lastHealth = status.resourcePercentage("health")
  ...
  self.keepTargetInRange = 200
    self.approachSpeed = config.getParameter("speed", 0)
...
function move()
    ...
    enforcePassDis = config.getParameter("enforcePassDis", 5)
    enforcePassTime = config.getParameter("enforcePassTime", 45)
    function postMove()
